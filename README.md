...
В полноэкранном просмотре доступны три похожие фотографии и возврат по истории. Кнопка `similar photos` сворачивает и раскрывает миниатюры, освобождая место для основного кадра; кнопка возврата видна только при раскрытых миниатюрах. Снимок, из которого только что пришли, исключается из предложений. Обычные стрелки и свайпы сохраняют порядок галереи. Если файл связей недоступен, обычный просмотр продолжает работать.
...
В админке описание, масштаб кадра, число людей и ручные теги сразу сохраняются локально в соответствующий `_meta` JSON. Контурные теги созданы моделью; при нажатии они становятся ручными.

Публикация устроена отдельно: фотографии можно отметить галочками и массово добавить в пул `publish` или `remove from site`; переключатель внутри редактора делает то же для одного кадра. Кнопка `review changes` показывает публикационный черновик со значениями «было → станет», а также метаданные, уже сохранённые локально и ожидающие синхронизации. `Apply & publish` применяет статусы публикации, пересобирает WebP и манифесты, создаёт Git-коммит и отправляет `main` в GitHub. Во время операции отдельное окно показывает живой журнал пересборки и push; при ошибке изменения сохраняются для повторной попытки.

Перед пересборкой админка создаёт отсутствующие sidecar-файлы для выбранных к публикации кадров и точечно запускает локальный анализ для опубликованных кадров, у которых не хватает описания, тегов, визуальных метрик или эмбеддинга. В боковой панели доступны режимы `shoots`, `all photos` и `problems`; последний собирает в одном потоке только опубликованные или подготовленные к публикации кадры с неполными метаданными и показывает конкретные причины на карточках. Неопубликованные исходники проблемами не считаются.

Раздел `tags` показывает частоту каждого тега, число опубликованных фотографий и происхождение manual/generated. Каждую строку можно развернуть в сетку связанных фотографий и открыть любой кадр в редакторе. Тег можно переименовать с автоматическим объединением совпадений или удалить после подтверждения. Переименованный тег становится ручным; массовые правки сразу сохраняются в исходные sidecar-файлы и попадают в очередь синхронизации сайта.
